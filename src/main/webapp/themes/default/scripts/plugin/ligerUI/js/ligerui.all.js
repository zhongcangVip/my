...
(function ($)
{
    ...
    window.liger = $.ligerui = {
        ...
        run: function (plugin, args, ext)
        {
            ...
            ext = $.extend({
                ...
                isStatic: false,
                hasElement: true,           //是否擁有element主體(比如drag、resizable等輔助性插件就不擁有)
                propertyToElemnt: null      //鏈接到element的屬性名
            }, ext || {});
            ...
            if (typeof args[0] == 'string')
            {
                ...
                {
                    var method = args[0];
                    if (!manager[method]) return; //不存在這個方法
    ...
    //組件基類
    //1,完成定義參數處理方法和參數屬性初始化的工作
    //2,完成定義事件處理方法和事件屬性初始化的工作
    ...
    $.extend($.ligerui.core.Component.prototype, {
        ...
        //觸發事件
        //data (可選) Array(可選)傳遞給事件處理函數的附加參數
    ...
    //界面組件基類, 
    //1,完成界面初始化:設置組件id並存入組件管理器池,初始化參數
    //2,渲染的工作,細節交給子類實現
    //@parm [element] 組件對應的dom element對像
    //@parm [options] 組件的參數
    ...
    $.ligerui.core.UIComponent.ligerExtend($.ligerui.core.Component, {
        ...
        _init: function ()
        {
            ...
            //讀取ligerui這個屬性，並加載到參數，比如 ligerui = "width:120,heigth:100"
            var p = this.options;
...
(function ($)
{
    ...
    $.ligerDefaults.ComboBox = {
        resize: true,           //是否調整大小
        isMultiSelect: false,   //是否多選
        isShowCheckBox: false,  //是否選擇復選框
        columns: false,       //表格狀態
        selectBoxWidth: false, //寬度
        selectBoxHeight: false, //高度
        onBeforeSelect: false, //選擇前事件
        onSelected: null, //選擇值事件 
        initValue: null,
        initText: null,
        valueField: 'id',
        textField: 'text',
        valueFieldID: null,
        slide: true,           //是否以動畫的形式顯示
        split: ";",
        data: null,
        tree: null,            //下拉框以樹的形式顯示，tree的參數跟LigerTree的參數一致 
        treeLeafOnly: true,   //是否只選擇葉子
        grid: null,              //表格
        onStartResize: null,
        onEndResize: null,
        hideOnLoseFocus: true,
        url: null,              //數據源URL(需返回JSON)
        onSuccess: null,
        onError: null,
        onBeforeOpen: null,      //打開下拉框前事件，可以通過return false來阻止繼續操作，利用這個參數可以用來調用其他函數，比如打開一個新窗口來選擇值
        render: null,            //文本框顯示html函數
        absolute: true         //選擇框是否在附加到body,並絕對定位
    };
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        onTextChange: function ()
        {
            ...
            else
            {
                ...
                g.usedDate = new Date(val); //記錄
                g.selectedDate = {
                    year: g.usedDate.getFullYear(),
                    month: g.usedDate.getMonth() + 1, //注意這裡
...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        cls: null,       //給dialog附加css class
        id: null,        //給dialog附加id
        buttons: null, //按鈕集合 
        isDrag: true,   //是否拖動
        width: 280,     //寬度
        height: null,   //高度，默認自適應 
        content: '',    //內容
        target: null,   //目標對象，指定它將以appendTo()的方式載入
        url: null,      //目標頁url，默認以iframe的方式載入
        load: false,     //是否以load()的方式加載目標頁的內容
        onLoaded: null,
        type: 'none',   //類型 warn、success、error、question
        left: null,     //位置left
        top: null,      //位置top
        modal: true,    //是否模態對話框
        name: null,     //創建iframe時 作為iframe的name和id 
        isResize: false, // 是否調整大小
        allowClose: true, //允許關閉
        opener: null,
        timeParmName: null,  //是否給URL後面加上值為new Date().getTime()的參數，如果需要指定一個參數名即可
        closeWhenEnter: null, //回車時是否關閉dialog
        isHidden: true,        //關閉對話框時是否只是隱藏，還是銷毀對話框
        show: true,          //初始化時是否馬上顯示
        title: '提示',        //頭部 
        showMax: false,                             //是否顯示最大化按鈕 
        showToggle: false,                          //是否顯示收縮窗口按鈕
        showMin: false,                             //是否顯示最小化按鈕
        slide: $.browser.msie ? false : true,        //是否以動畫的形式顯示 
        fixedType: null,            //在固定的位置顯示, 可以設置的值有n, e, s, w, ne, se, sw, nw
        showType: null             //顯示類型,可以設置為slide(固定顯示時有效)
    };
    $.ligerDefaults.DialogString = {
        titleMessage: '提示',                     //提示文本標題
        ok: '確定',
        yes: '是',
        no: '否',
        cancel: '取消',
        waittingMessage: '正在等待中,請稍候...'
    };
...
(function ($)
{
    ...
    $.ligerDefaults.Drag = {
        ...
        onStopDrag: false,
        handler: null,
        //代理 拖動時的主體,可以是'clone'或者是函數,放回jQuery 對像
        proxy: true,
        revert: false,
        animate: true,
        onRevert: null,
        onEndRevert: null,
        //接收區域 jQuery對像或者jQuery選擇字符
        receive: null,
        //進入區域
        onDragEnter: null,
        //在區域移動
        ...
        onDrop: null,
        disabled: false,
        proxyX: null,     //代理相對鼠標指針的位置,如果不設置則對應target的left
        proxyY: null
    };
...
(function ($)
{
    ...
    $.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
        ...
        //增加分組
        //@parm [jgroup] jQuery對像(主分組的table dom元素)
        ...
        //設置規則
        //@parm [group] 分組數據
        //@parm [jgruop] 分組table dom jQuery對像
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        width: 'auto',                          //寬度值
        height: 'auto',                          //寬度值
        columnWidth: null,                      //默認列寬度
        resizable: true,                        //table是否可伸縮
        url: false,                             //ajax url
        usePager: true,                         //是否分頁
        page: 1,                                //默認當前頁 
        pageSize: 10,                           //每頁默認的結果數
        pageSizeOptions: [10, 20, 30, 40, 50],  //可選擇設定的每頁結果數
        parms: [],                         //提交到服務器的參數
        columns: [],                          //數據源
        minColToggle: 1,                        //最小顯示的列
        dataType: 'server',                     //數據源：本地(local)或(server),本地是將讀取p.data。不需要配置，取決於設置了data或是url
        dataAction: 'server',                    //提交數據的方式：本地(local)或(server),選擇本地方式時將在客服端分頁、排序。 
        showTableToggleBtn: false,              //是否顯示'顯示隱藏Grid'按鈕 
        switchPageSizeApplyComboBox: false,     //切換每頁記錄數是否應用ligerComboBox
        allowAdjustColWidth: true,              //是否允許調整列寬     
        checkbox: false,                         //是否顯示復選框
        allowHideColumn: true,                 //是否顯示'切換列層'按鈕
        enabledEdit: false,                      //是否允許編輯
        isScroll: true,                         //是否滾動
        onDragCol: null,                       //拖動列事件
        onToggleCol: null,                     //切換列事件
        onChangeSort: null,                    //改變排序事件
        onSuccess: null,                       //成功獲取服務器數據的事件
        onDblClickRow: null,                     //雙擊行事件
        onSelectRow: null,                    //選擇行事件
        onUnSelectRow: null,                   //取消選擇行事件
        onBeforeCheckRow: null,                 //選擇前事件，可以通過return false阻止操作(復選框)
        onCheckRow: null,                    //選擇事件(復選框) 
        onBeforeCheckAllRow: null,              //選擇前事件，可以通過return false阻止操作(復選框 全選/全不選)
        onCheckAllRow: null,                    //選擇事件(復選框 全選/全不選)
        onBeforeShowData: null,                  //顯示數據前事件，可以通過reutrn false阻止操作
        onAfterShowData: null,                 //顯示完數據事件
        onError: null,                         //錯誤事件
        onSubmit: null,                         //提交前事件
        dateFormat: 'yyyy-MM-dd',              //默認時間顯示格式
        InWindow: true,                        //是否以窗口的高度為準 height設置為百分比時可用
        statusName: '__status',                    //狀態名
        method: 'post',                         //提交方式
        async: true,
        fixedCellHeight: true,                       //是否固定單元格的高度
        heightDiff: 0,                         //高度補差,當設置height:100%時，可能會有高度的誤差，可以通過這個屬性調整
        cssClass: null,                    //類名
        root: 'Rows',                       //數據源字段名
        record: 'Total',                     //數據源記錄數字段名
        pageParmName: 'page',               //頁索引參數名，(提交給服務器)
        pagesizeParmName: 'pagesize',        //頁記錄數參數名，(提交給服務器)
        sortnameParmName: 'sortname',        //頁排序列名(提交給服務器)
        sortorderParmName: 'sortorder',      //頁排序方向(提交給服務器)
        onReload: null,                    //刷新事件，可以通過return false來阻止操作
        onToFirst: null,                     //第一頁，可以通過return false來阻止操作
        onToPrev: null,                      //上一頁，可以通過return false來阻止操作
        onToNext: null,                      //下一頁，可以通過return false來阻止操作
        onToLast: null,                      //最後一頁，可以通過return false來阻止操作
        allowUnSelectRow: false,           //是否允許反選行 
        alternatingRow: true,           //奇偶行效果
        mouseoverRowCssClass: 'l-grid-row-over',
        enabledSort: true,                      //是否允許排序
        rowAttrRender: null,                  //行自定義屬性渲染器(包括style，也可以定義)
        groupColumnName: null,                 //分組 - 列名
        groupColumnDisplay: '分組',             //分組 - 列顯示名字
        groupRender: null,                     //分組 - 渲染器
        totalRender: null,                       //統計行(全部數據)
        delayLoad: false,                        //初始化時是否不加載
        where: null,                           //數據過濾查詢函數,(參數一 data item，參數二 data item index)
        selectRowButtonOnly: false,            //復選框模式時，是否只允許點擊復選框才能選擇行
        onAfterAddRow: null,                     //增加行後事件
        onBeforeEdit: null,                      //編輯前事件
        onBeforeSubmitEdit: null,               //驗證編輯器結果是否通過
        onAfterEdit: null,                       //結束編輯後事件
        onLoading: null,                        //加載時函數
        onLoaded: null,                          //加載完函數
        onContextmenu: null,                   //右擊事件
        whenRClickToSelect: false,                //右擊行時是否選中
        contentType: null,                     //Ajax contentType參數
        checkboxColWidth: 27,                  //復選框列寬度
        detailColWidth: 29,                     //明細列寬度
        clickToEdit: true,                      //是否點擊單元格的時候就編輯
        detailToEdit: false,                     //是否點擊明細的時候進入編輯
        onEndEdit: null,
        minColumnWidth: 80,
        tree: null,                            //treeGrid模式
        isChecked: null,                       //復選框 初始化函數
        frozen: true,                          //是否固定列
        frozenDetail: false,                    //明細按鈕是否在固定列中
        frozenCheckbox: true,                  //復選框按鈕是否在固定列中
        detailHeight: 260,
        rownumbers: false,                         //是否顯示行序號
        frozenRownumbers: true,                  //行序號是否在固定列中
        rownumbersColWidth: 26,
        colDraggable: false,                       //是否允許表頭拖拽
        rowDraggable: false,                         //是否允許行拖拽
        rowDraggingRender: null,
        autoCheckChildren: true,                  //是否自動選中子節點
        onRowDragDrop: null,                    //行拖拽事件
        //rowHeight: 22,                           //行默認的高度
        headerRowHeight: 23,                    //表頭行的高度
        toolbar: null,                           //工具條,參數同 ligerToolbar的
        headerImg: null                        //表格頭部圖標
    };
    $.ligerDefaults.GridString = {
        errorMessage: '發生錯誤',
        pageStatMessage: '顯示從{from}到{to}，總 {total} 條 。每頁顯示：{pagesize}',
        pageTextMessage: 'Page',
        loadingMessage: '加載中...',
        findTextMessage: '查找',
        noRecordMessage: '沒有符合條件的記錄存在',
        isContinueByDataChanged: '數據已經改變,如果繼續將丟失數據,是否繼續?',
        cancelMessage: '取消',
        saveMessage: '保存',
        applyMessage: '應用',
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.changedCells = {};
            g.editors = {};                 //多編輯器同時存在
            g.editor = { editing: false };  //單編輯器,配置clickToEdit
            if (p.height == "auto")
            {
        ...
        /*
        @description 使一行進入編輯狀態
        @param  {rowParm} rowindex或者rowdata
        @param {containerBulider} 編輯器填充層構造器
        */
        ...
        /*
        @param  {arg} column index、column name、column、單元格
        @param  {value} 值
        @param  {rowParm} rowindex或者rowdata
        */
        ...
        getRow: function (rowParm)
        {
            ...
            else if (typeof (rowParm) == "object" && rowParm.nodeType == 1 && !rowParm['__id']) //dom對像
            {
        ...
        //移動數據(樹)
        //@parm [parentdata] 附加到哪一個節點下級
        //@parm [neardata] 附加到哪一個節點的上方/下方
        ...
        toggle: function (rowParm)
        {
            ...
            if (linkbtn.hasClass("l-grid-tree-link-close")) //收縮
            {
        ...
        _initColumns: function ()
        {
            ...
            //設置id、pid、level、leaf，返回葉節點數,如果是葉節點，返回1
        ...
        _initBuildGridHeader: function ()
        {
            ...
            {
                var columns = g.getColumns(level);           //獲取level層次的列集合
                var islast = level == g._columnMaxLevel;     //是否最末級
        ...
        _fillGridBody: function (data, frozen)
        {
            ...
            if (g.enabledGroup()) //啟用分組模式
            {
                var groups = []; //分組列名數組
                var groupsdata = []; //切成幾塊後的數據
                g.groups = groupsdata;
        ...
        _getSrcElementByEvent: function (e)
        {
            ...
            var r = {
                ...
                header: fn("l-panel-header"), //標題
                gridheader: fn("l-grid-header"), //表格頭 
                gridbody: fn("l-grid-body"),
                total: fn("l-panel-bar-total"), //總匯總 
        ...
        _onClick: function (e)
        {
            ...
            else if (src.row && g.enabledCheckbox()) //復選框選擇行
            {
                //復選框
        ...
        select: function (rowParm)
        {
            ...
            if (!g.enabledCheckbox() && !g.ctrlKey) //單選
            {
...
(function ($)
{
    ...
    $.ligerDefaults.Layout = {
        ...
        isLeftCollapse: false,      //初始化時 左邊是否隱藏
        isRightCollapse: false,     //初始化時 右邊是否隱藏
        allowLeftCollapse: true,      //是否允許 左邊可以隱藏
        allowRightCollapse: true,     //是否允許 右邊可以隱藏
        allowLeftResize: true,      //是否允許 左邊可以調整大小
        allowRightResize: true,     //是否允許 右邊可以調整大小
        allowTopResize: true,      //是否允許 頭部可以調整大小
        allowBottomResize: true,     //是否允許 底部可以調整大小
        space: 3, //間隔 
        onEndResize: null,          //調整大小結束事件
        minLeftWidth: 80,            //調整左側寬度時的最小允許寬度
        minRightWidth: 80           //調整右側寬度時的最小允許寬度
    };
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            if ($("> div[position=left]", g.layout).length > 0)
            {
                ...
                var leftHref = g.left.content.attr("href");
                if (lefttitle)
                {
                    ...
                    if(!leftHref){
                    	...
                    	var leftHrefContent = "<a href='"+leftHref+"'>" + lefttitle + "</a>";
                    	$(".l-layout-header-inner", g.left.header).html(leftHrefContent);
                    }
                }
                //set width
        ...
        _start: function (dragtype, e)
        {
            ...
            g.layout.lock.show();
            if ($.browser.msie || $.browser.safari) $('body').bind('selectstart', function () { return false; }); // 不能選擇
...
(function ($)
{
    ...
    $.ligerDefaults.Spinner = {
        type: 'float',     //類型 float:浮點數 int:整數 time:時間
        isNegative: true, //是否負數
        decimalplace: 2,   //小數位 type=float時起作用
        step: 0.1,         //每次增加的值
        interval: 50,      //間隔，毫秒
        onChangeValue: false,    //改變值事件
        minValue: null,        //最小值
        maxValue: null,         //最大值
        disabled: false
    };
...
(function ($)
{
    ...
    $.ligerDefaults.Tab = {
        ...
        changeHeightOnResize: false,
        contextmenu: true,
        dblClickToClose: false, //是否雙擊時關閉
        dragToMove: false,    //是否允許拖動時改變tab項的位置
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        //設置tab按鈕(左和右),顯示返回true,隱藏返回false
        ...
        moveToPrevTabItem: function ()
        {
            ...
            var leftList = new Array(); //記錄每個tab的left,由左到右
        ...
        moveToNextTabItem: function ()
        {
            ...
            var mainwidth = g.tab.width();
            var leftList = new Array(); //記錄每個tab的left,由右到左 
...
(function ($)
{
    ...
    $.ligerDefaults.TextBox = {
        ...
        disabled: false,
        value: null,     //初始化值 
        nullText: null,   //不能為空時的提示
        digits: false,     //是否限定為數字輸入框
        number: false    //是否限定為浮點數格式輸入框
    };
...
(function ($)
{
    ...
    $.ligerDefaults.Tree = {
        ...
        textFieldName: 'text',
        attribute: ['id', 'url'],
        treeLine: true,            //是否顯示line
        nodeWidth: 90,
        statusName: '__status',
        isLeaf: null,              //是否子節點的判斷函數
        single: false,               //是否單選
        ...
        topParentIDValue: 0,
        onBeforeAppend: function () { },        //加載數據前事件，可以通過return false取消操作
        onAppend: function () { },             //加載數據時事件，對數據進行預處理以後
        onAfterAppend: function () { },         //加載數據完事件
        slide: true,          //是否以動畫的形式顯示
        iconFieldName: 'icon',
        nodeDraggable: false,             //是否允許拖拽
        nodeDraggingRender: null,
        btnClickToToggleOnly: true     //是否點擊展開/收縮 按鈕時才有效
    };
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        //@parm [treeNode] dom節點(li)、節點數據 或者節點 dataindex
        ...
        //@parm [domnode] dom節點(li)、節點數據 或者節點 dataindex
        ...
        //增加節點集合
        //@parm [newdata] 數據集合 Array
        //@parm [parentNode] dom節點(li)、節點數據 或者節點 dataindex
        //@parm [nearNode] 附加到節點的上方/下方(非必填)
        //@parm [isAfter] 附加到節點的下方(非必填)
        ...
        //@parm [nodeParm] dom節點(li)、節點數據 或者節點 dataindex
        ...
        arrayToTree: function (data, id, pid)      //將ID、ParentID這種數據格式轉換為樹格式
        {
            if (!data || !data.length) return [];
            var targetData = [];                    //存儲數據的容器(返回) 
            var records = {};
            var itemLength = data.length;           //數據集合的個數
